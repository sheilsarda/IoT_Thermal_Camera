...
void printImage(){
	...
	snprintf(img_msg, 255, "{\"d\":{\"img\":{\"one\":%ld,\"two\":%ld,\"three\":%ld,\"four\":%ld}}}", buffer[loop], buffer[loop+1], buffer[loop+2], buffer[loop+3]);
		...
		if(strncmp((char *) msgData->topicName->lenstring.data, STOP_TOPIC, msgData->message->payloadlen) == 0)
		{
			if(strncmp((char *)msgData->message->payload, LED_TOPIC_LED_OFF, msgData->message->payloadlen) == 0)
			{
				for (int i = 5; i >= 0; i--)
				{
					...
					snprintf(servo_msg, 63, "{\"d\":{\"servo\":%d}}", 36*i);
